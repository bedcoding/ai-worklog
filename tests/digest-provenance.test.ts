...
let logFile = ''
/** 스캔이 실제 ~/.claude를 읽지 않도록 주는 빈 로그 디렉토리 */
let emptyDir = ''
 ...
 * 나중에 덧붙기 때문이다. 그래서 시각이 아니라 소스 파일의 mtime을 본다.
 *
 * 낡았으면 곧바로 다시 읽는다. state가 'scanned' 면 캐시를 쓰지 않기로 판정한 것이다.
...
describe('원본 내역 캐시를 쓸지 판정', () => {
  ...
  it('하루가 끝난 뒤에 만들었어도 소스가 바뀌면 다시 읽는다', async () => {
    // 8/22가 정확히 이 경우였다. builtAt은 하루 끝난 뒤인데 소스가 그 뒤에 바뀌었다
    await writeJsonAtomic(
      dayDigestPath('2026-07-15'),
      digestWith('2026-07-15', AFTER_DAY, [{ path: logFile, mtimeMs: AT }])
    )
  ...
  it('소스 파일이 사라진 것은 낡음으로 보지 않는다', async () => {
    // 다시 읽으면 결과가 오히려 줄어든다. Claude Code가 옛 로그를 지운 경우다
    await writeJsonAtomic(
      dayDigestPath('2026-07-15'),
  ...
  it('sources가 없는 옛 캐시는 쓰지 않는다', async () => {
    // 판정할 수 없으면 믿지 않는다. 한 번 다시 읽으면 sources가 붙어
