...
describe('구간 커서', () => {
  it('주간은 그 날짜를 담는 월~일 구간을 만든다', () => {
    // 2026-08-23 은 일요일이므로 8/17(월)~8/23(일) 주에 속한다
  ...
  it('기간 요약 캐시 키를 만든다', () => {
    // key 가 캐시 파일명이 되므로 형식이 바뀌면 기존 요약을 못 찾는다
...
describe('단위 전환', () => {
  it('단위를 바꿔도 구간이 밀리지 않는다', () => {
    ...
    // 2026-07-01 은 수요일이라 그 주가 06-29 에 시작하고, 그 날짜로 달을 다시 잡으면
    // 7월이 6월이 됐다.
...
describe('구간 이동', () => {
  ...
  it('말일에서 옮겨도 달을 건너뛰지 않는다', () => {
    // 1/31 에 한 달을 더해 2/31 을 만들면 3월로 넘어가 버린다
