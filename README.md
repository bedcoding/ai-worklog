...
**Claude Code 사용 내역으로 일일·주간·월간 업무 요약을 자동 생성하는 macOS·Windows 트레이 앱**

오늘 무슨 일을 했는지 적어야 하나요?
...
- **프롬프트 커스텀**: 요약 프롬프트 2종을 설정 탭에서 자유롭게 수정
- **자동 정리**: 오래된 원본 추출 캐시는 보관 기간(기본 12개월)이 지나면 자동 삭제

## 프라이버시 (중요)

업무 기록을 다루는 도구이니, 스스로도 감사 가능하게 만들었습니다.

1. **업무 기록은 어떤 경우에도 밖으로 나가지 않습니다.** `src/`에 네트워크 코드가 없음을
   CI가 강제합니다 (`scripts/check-no-network.mjs`, 릴리스마다 실행).
   예외는 `src/main/update-check.ts` 한 파일뿐입니다. 주 1회 GitHub에 최신 버전 번호를
   묻는 GET 요청 하나이고 보내는 데이터는 없습니다. 그 파일이 허용된 주소 외에 접속하면
   CI가 막습니다. 새 버전을 알리기만 하고 내려받거나 설치하지는 않습니다.
   확인을 원치 않으면 오프라인에서도 나머지 기능은 그대로 동작합니다.
...
5. **최소 의존성**: 런타임 의존성은 `react`, `react-dom` 둘뿐이고, 릴리스 바이너리는
...
4. 메뉴바의 문서 아이콘 클릭 → 설정 탭에서 claude 경로·모델 확인
...
6. 아이콘 클릭 → 설정 탭에서 claude 경로·모델 확인
...
| 월간 요약 | 한 달 업무 요약 생성 → 복사. 두 버튼이 claude를 몇 번 부르는지 숫자로 표시 |
| 설정 | 로그인 시 자동 시작, claude 경로/모델, 매일 자동 요약, 보관 기간, 프롬프트 템플릿 |
